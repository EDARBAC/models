globals[
  ...
  grabbed-vertex   ; a user-selected vertex
...
undirected-link-breed [ sides side ]

to setup
  clear-all
  ask patches [            ; creates lattice with dots at each patch
    set pcolor 0
    sprout-dots 1 [
      set color 45
      set size 0.2
...
to draw-triangle
  ...
  create-vertices 3 [
    ...
    set size 0.5
    set color 115
    create-sides-with other vertices         ; creates sides to form a triangle
    ask sides [
      set color 85
      set thickness 0.1
    ]
  ...
  ask side [who] of v1 [who] of v2 [ set label "S3  " ]
  ask side [who] of v1 [who] of v3 [ set label "S2  " ]
  ask side [who] of v2 [who] of v3 [ set label "S1  " ]
  ask sides [ set label-color white]
display
end


to go
  if last-action != action [         ; Checks if user changes to a action
    set previous-click nobody        ; If so, clear any incomplete segments
    set last-action action
    ask dots [ reset ]               ; and reset dot size and color
  ]

  ifelse action = "Draw Segment" [
    draw-segment
  ] [
    ifelse action = "Modify Triangle" [
      mod-triangle
    ] [
...
to mod-triangle
  ...
  if mouse-down? and not mouse-was-down? [
    set grabbed-vertex one-of vertices with [distance one-of users < 0.4]
  ]
  ; while the mouse is down,
  ; update location of grabbed vertex to the lattice dot closest to the mouse
  if-else mouse-down?  [
    if grabbed-vertex != nobody [
      if-else mouse-was-down? [
        ...
        ask grabbed-vertex [setxy v-xcor v-ycor]
        ...
        ask grabbed-vertex [ set color white   set size 0.75 ]
      ]
...
; When user clicks on two dots, they are joined by a link.
to draw-segment
...
to reset-previous-click  ; Reset size of previous dot clicked and resets previous-click variable
  if previous-click != nobody [ ask previous-click [ set size 0.2 ] ]
  set previous-click nobody
end
...
action
action
"Modify Triangle" "Draw Segment" "Delete Segment"
0
